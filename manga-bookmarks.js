...
async function fetchBookmarksFromSupabase() {
  ...
  if (error) {
    ...
    console.error('[manga-bookmarks] fetch failed:', error);
    window.alert('漫画ブックマーク取得に失敗しました: ' + summary);
    return;
  }
...
async function upsertBookmarkToSupabase(item) {
  const { data, error } = await window.supabaseClient.from('manga_bookmarks').upsert(
    {
      id: item.id,
      url: item.url,
      title: item.title,
...
async function deleteItem(id) {
  ...
  const { error } = await window.supabaseClient.from('manga_bookmarks').delete().eq('id', id);
  if (error) {
    const summary = formatSupabaseError(error);
    console.error('[manga-bookmarks] delete failed:', error);
...
async function addItem(event) {
  ...
  try {
    ...
    console.error('[manga-bookmarks] save failed:', error);
    const summary = formatSupabaseError(error);
