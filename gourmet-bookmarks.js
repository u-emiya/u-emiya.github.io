...
async function fetchBookmarksFromSupabase() {
  ...
  const { data, error } = await window.supabaseClient
    .from('gourmet_bookmarks')
    .select('*')
    .order('created', { ascending: false });

  if (error) {
    const summary = formatSupabaseError(error);
    console.error('[gourmet-bookmarks] fetch failed:', error);
    window.alert('グルメブックマーク取得に失敗しました: ' + summary);
    return;
  }
...
async function upsertBookmarkToSupabase(item) {
  const { data, error } = await window.supabaseClient.from('gourmet_bookmarks').upsert(
    {
      id: item.id,
      url: item.url,
      title: item.title,
...
async function deleteItem(id) {
  ...
  const { error } = await window.supabaseClient.from('gourmet_bookmarks').delete().eq('id', id);
  if (error) {
    const summary = formatSupabaseError(error);
    console.error('[gourmet-bookmarks] delete failed:', error);
...
async function addItem(event) {
  ...
  try {
    ...
    console.error('[gourmet-bookmarks] save failed:', error);
    const summary = formatSupabaseError(error);
